...
export const useAsyncActionsModal = ({ onSuccess, onError }: {onSuccess?: () => void, onError?: () => void}) => {
  const cancelButtonRef = useRef(null)
  const [actions, setActions] = useState<Action[] | undefined>()
  const [currentStep, setCurrentStep] = useState(0)
  const [status, setStatus] = useState<Status[]>(Array(actions?.length || 0).fill("idle"))
  ...
  const modal = (
    <Transition.Root show={!!actions} as={Fragment}>
      <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={reset}>
        <Transition.Child
          as={Fragment}
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg text-left shadow-xl bg-base-300 transition-all sm:my-8 sm:w-full sm:max-w-lg">
                  ...
                  <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center flex flex-col gap-2 sm:mt-0 sm:ml-4 sm:text-left">
                      ...
                      <table className='table w-full rounded'>
                        ...
                        <tbody>
                          {actions?.map((e, i) => (<tr key={e.description + i}>
                            <th>{statusSymbol(status[i])}</th>
                            <th><div className="flex flex-col gap-1">
                              <div className="">{e.title || ""} {i}</div>
                              <div className="text-xs opacity-60">{e.description || ""}</div>
                            </div></th>
                          </tr>)) || null}
                        </tbody>
                      </table>
                ...
                <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  {currentStep === actions?.length || 0 ? <button
                    type="button"
