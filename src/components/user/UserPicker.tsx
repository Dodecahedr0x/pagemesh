...
export function UserPicker() {
  ...
  const fetchUsers = useCallback(
    async function () {
      ...
      setUsers(
        (await sdk.user.getUserAccountsByAuthority(publicKey)).sort((a, b) =>
          a.cl_pubkey > b.cl_pubkey ? 1 : -1
        )
      );
    },
    [sdk, publicKey]
  ...
  if (!users || users.length === 0) {
    return (
      <div className="btn btn-primary btn-sm" onClick={handleCreateUser}>
        Create user
      </div>
    );
  }
      ...
      {({ open }) => (
        <>
          ...
          >
            ...
            <div>
              {user && user.authority === publicKey.toString()
                ? shortAddress(user.cl_pubkey)
                : "Pick a user"}
            </div>
          </Menu.Button>
          <Menu.Items
          ...
          >
            {users.map((u) => (
              <Menu.Item
                key={u.cl_pubkey.toString()}
                disabled={u.cl_pubkey === user?.cl_pubkey}
              >
                <button
